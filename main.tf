...
resource "azurerm_linux_web_app" "webapplinux" {
  ...
  site_config {
    ...
    application_stack {
      ...
      java_server         = var.java_version != null && var.java_server != null && var.java_server_version != null ? var.java_server : null
      java_server_version = var.java_version != null && var.java_server != null && var.java_server_version != null ? var.java_server_version : null
      ...
      ruby_version = var.ruby_version != null ? var.ruby_version : null
...
resource "azurerm_linux_web_app_slot" "webapplinux_slot" {
  count                     = var.create_slot == true ? 1 : 0
  app_service_id            = azurerm_linux_web_app.webapplinux.id
...
module "app_service_private_endpoint" {
  ...
  resource_group_name                 = var.resource_group_name
  private_connection_resource_id      = azurerm_linux_web_app.webapplinux.id
...
#  virtual_network_resource_group_name = var.virtual_network_resource_group_name
#  resource_group_name                 = var.resource_group_name
#  private_connection_resource_id      = azurerm_linux_web_app_slot.webapplinux_slot[0].id
#  subresourceType                     = "sites-staging"
#}

resource "azurerm_monitor_diagnostic_setting" "webapplinux" {
  count                      = var.enable_logs_analytics == true ? 1 : 0
  name                       = "diagnostic"
  target_resource_id         = azurerm_linux_web_app.webapplinux.id
  log_analytics_workspace_id = var.workspace_id

  depends_on = [azurerm_linux_web_app.webapplinux]
